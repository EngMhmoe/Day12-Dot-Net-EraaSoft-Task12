...
namespace WebApplication2.Controllers
{
    public class AppointmentController : Controller
    {
        ...
        [HttpGet]
        public IActionResult Create(int doctorId) // id = DoctorId
        {
            var doctor = _context.Doctors.Find(doctorId);
            if (doctor == null) return NotFound();

            ViewBag.Doctor = doctor; // عشان يظهر الاسم فوق
            ViewBag.TimeSlots = new List<string> { "09:00", "09:30", "10:00", "10:30", "11:00" }; // المواعيد

            var appointment = new Appointment { DoctorId = doctorId }; // مهم جدا
        ...
        private List<TimeSpan> GetTimeSlots()
        {
            ...
            for (int i = 9; i <= 16; i++) // 9 AM to 4 PM
            {
                slots.Add(new TimeSpan(i, 0, 0));
                slots.Add(new TimeSpan(i, 30, 0));
            }
            slots.Add(new TimeSpan(16, 30, 0)); // 4:30 PM
            return slots;
        }
    }
}
